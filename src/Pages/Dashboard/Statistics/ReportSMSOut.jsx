...
const ReportSMSOut = () => {
  const [selectedTab, setSelectedTab] = useState("Source Address");
  const tabs = [
      ...
      <div className="bg-blue-50 min-h-screen p-6">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-wrap items-center gap-4 mb-6">
            ...
            <div className="flex items-center space-x-2">
              <label className="text-gray-600">A2P Campaign</label>
              <select className="bg-white border border-gray-300 rounded-md p-2">
                <option>Any</option>
          ...
          <div className="w-full bg-white h-[50vh] rounded ">
            ...
            <div className="mt-4 bg-white h-full w-full flex justify-center items-center">
              {selectedTab === "Source Address" && (
                <p>No data for selected timeframe</p>
              )}
              {selectedTab === "Destination Country" && (
                <p>Destination Country content goes here.</p>
              )}
              {selectedTab === "SMS OUT Trunk" && (
                <p>SMS OUT Trunk content goes here.</p>
              )}
              {selectedTab === "A2P Campaign" && (
                <p>A2P Campaign content goes here.</p>
              )}
            </div>
          </div>
        </div>
